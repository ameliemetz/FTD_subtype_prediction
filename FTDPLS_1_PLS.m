    %% this code will prepare clean tables for baseline data (DBM values and cognitive tests)
    %% and run a PLS to get latent variables combining brain atrophy and cognition data that maximally covary with each other
    ...
    %max: includes 16 cognitive scores/demographic data
    %min: includes BNT, CDR, and demographic data

    version = 'min';


    %% pick cohort: 'all', 'CDR-matched', 'age-matched', only 'UCSF' site
    %pick cohort to avoid certain confounding effects
    %all: entire NIFD dataset
    %CDR-matched: only participants with total CDR score under 1.5 (avoid
    %disease severity effects)
    %age-matched: nfvPPA group is older than other groups, ie this will
    %only include age-matched subset of participants (for age-matching see
    %R code)
    %UCSF: exclude Mayo and MGH site with few participants
    ...
    switch cohort
        ...
        case 'age-matched' %see age-matching script
            Info=readtable('Info_agematched.csv','Delimiter',',');
            Info(:,'sum_PPVT')=[];
        case 'CDR-matched'
            Info=readtable('NIFD_MRI_Measures.csv','Delimiter',',');
            Info=Info(Info.CDR_TOT<1.5,:);
        case 'UCSF'
            Info=readtable('NIFD_MRI_Measures.csv','Delimiter',',');
            Info=Info(strcmp(Info.SITE, 'UCSF'), :);
        otherwise
            error('Invalid cohort: %s', cohort);
    end
    ...
    %% making an overall FTD group with all 3 subgroups in it
    ...
    %save list of diagnoses/IDs and table of all included participants
    ...
    %% W-scores: Regress out effects of age, sex, and education based on healthy controls (if chosen above)
    ...
    %% Extracting the names of the variables (for plotting later)
    ...
    %% Partial Least Squares - we are using this: https://github.com/McIntosh-Lab/PLS
    ...
    %% save csv files for LVs (for plots and follow-up analyses)
